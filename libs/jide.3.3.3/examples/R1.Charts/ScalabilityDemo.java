...
import com.jidesoft.chart.SingleImageLazyChartRenderer;
...
public class ScalabilityDemo extends AbstractDemo {
    ...
    private Chart createDemo() {
        chart = new Chart();
        // Use this line if you want to use one in-memory image for all models (as for earlier JIDE releases)
        // By default one image is used per model, which can help make applications faster in many use cases
        // but also uses more memory
        //chart.setLazyChartRenderer(new SingleImageLazyChartRenderer(chart));
        chart.setBorder(new EmptyBorder(10, 0, 0, 10));
    ...
    class ModelCreator extends SwingWorker<ChartModel, Void> {
        ...
        protected ChartModel doInBackground() throws Exception {
            ...
            ChartModel currentModel = chart.getModel();
            if (currentModel != null) {
                model.setName(currentModel.getName());
            }
        ...
        protected void done() {
            try {
                ...
                if (currentModel == null) {
                    ...
                    chart.replaceModel(currentModel, m);
                }
                style.setPointSize(3);
                // Box shapes are simpler to draw than circles so are better suited to large data sets
    ...
    public String getDescription() {
        return "This example shows efficient handling of large data sets. " +
                "Rendering large data sets can take some time, but with JIDE Charts' advanced rendering approach, the" +
