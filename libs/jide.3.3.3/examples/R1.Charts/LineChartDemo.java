...
public class LineChartDemo extends AbstractDemo {
    ...
    private JPanel createDemo() {
        chart = new Chart();
        sineModel = createModel("Sine", true);
        cosineModel = createModel("Cosine", false);
        ...
        final ChartCrossHair crossHair = new ChartCrossHair(chart);
        chart.addMouseListener(crossHair);
        chart.addMouseMotionListener(crossHair);
        chart.addDrawable(crossHair);

        // If you are using the ChartValueReporter without a ChartCrossHair you can pass a Chart instance
        // to the constructor, but when used together it is better to pass the crossHair to minimize the number
        // of MouseMotionListeners.
        final ChartValueReporter valueReporter = new ChartValueReporter(crossHair);

        JPanel reporterPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        // The model reporter label demonstrates how you can get hold of the model that is underneath the
        // cross hair. A property change event is fired by the ChartCrossHair object whenever the model changes
        final JLabel modelReporter = new JLabel();
        reporterPanel.add(modelReporter);
        crossHair.addPropertyChangeListener(new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent evt) {
                if (ChartCrossHair.PROPERTY_OBSERVED_MODEL.equals(evt.getPropertyName())) {
                    ChartModel m = crossHair.getObservedModel();
                    modelReporter.setText(m == null ? "" : m.getName());
                }
            }
        });
        
        reporterPanel.add(valueReporter);
        demoPanel.add(reporterPanel, BorderLayout.SOUTH);
    ...
    private ChartModel createModel(String name, boolean sine) {
