...
public class TreeTableScrollPaneDemo extends AbstractDemo {
    ...
    public String getName() {
        return "TableScrollPane Demo (TreeTable)";
    }
    ...
    private TableScrollPane createTablePane() {
        ...
        _scrollPane = new TableScrollPane(new FilterableTreeTableModel(_model), true) {
            @Override
            ...
            protected TableModel createSortableTableModel(MultiTableModel tableModel, boolean sortable) {
                return new StyledSortableTreeTableModel(tableModel);
            }
        };

        AutoFilterTableHeader header = new AutoFilterTableHeader(_scrollPane.getRowHeaderTable());
        _scrollPane.getRowHeaderTable().setTableHeader(header);
        header.setAutoFilterEnabled(true);
        header.setUseNativeHeaderRenderer(true);
        header = new AutoFilterTableHeader(_scrollPane.getMainTable());
        _scrollPane.getMainTable().setTableHeader(header);
        header.setAutoFilterEnabled(true);
        header.setUseNativeHeaderRenderer(true);
    ...
    private void outputToCsv(ActionEvent e) {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                CsvTableScrollPaneUtils.export(_scrollPane, chooser.getSelectedFile().getAbsolutePath());
            }
    ...
    public class StyledSortableTreeTableModel extends SortableTreeTableModel implements StyleModel {
        CellStyle style1 = new CellStyle();
        CellStyle style2 = new CellStyle();

        public StyledSortableTreeTableModel(TableModel model) {
            super(model);
            style1.setBackground(new Color(253, 253, 244));
            style2.setBackground(new Color(244, 253, 244));
        }
        ...
        public CellStyle getCellStyleAt(int row, int column) {
            ...
            Row ancestor = getAncestor(rowAt);
            int index = TreeTableUtils.getChildIndex(this, (Row) ancestor.getParent(), ancestor);
            return (index % 2 == 0) ? style1 : style2;
        }

        public Row getAncestor(Row row) {
            if (row.getParent() instanceof RootExpandableRow) {
                return row;
            }
            else {
                return getAncestor((Row) row.getParent());
            }
        }
    }
}
